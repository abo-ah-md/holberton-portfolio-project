                ...
                <div className="w-full mx-auto px-4 md:px-6 py-3 flex items-center justify-between gap-4">
                    ...
                    {!isReviewer && (
                        <nav className="hidden md:flex items-center gap-1">
                            ...
                            <Link
                                to="/"
                                className={`flex flex-col items-center gap-1 px-4 py-2 rounded-lg hover:bg-white/10 transition-all duration-200 ${location.pathname === '/' ? 'border-b-2 border-brand-primary' : ''
                                    }`}
                            >
                                <Home size={20} className="text-white" />
                                <span className="text-xs text-white">الرئيسية</span>
                            </Link>

                            {/* Books */}
                            <Link
                                to="/marketplace"
                                className={`flex flex-col items-center gap-1 px-4 py-2 rounded-lg hover:bg-white/10 transition-all duration-200 ${location.pathname === '/marketplace' ? 'border-b-2 border-brand-primary' : ''
                                    }`}
                            >
                                <BookOpen size={20} className="text-white" />
                                <span className="text-xs text-white">الكتب</span>
                            </Link>

                            {/* About */}
                            <button
                                onClick={() => scrollToSection('about-us')}
                                className="flex flex-col items-center gap-1 px-4 py-2 rounded-lg hover:bg-white/10 transition-all duration-200"
                            >
                                <Info size={20} className="text-white" />
                                <span className="text-xs text-white">من نحن</span>
                            </button>
                        </nav>
                    )}
                    ...
                    <div className="flex items-center gap-1">
                        ...
                        {!isReviewer && (
                            <div className="hidden md:flex items-center gap-1">
                                ...
                                {user?.role === 'ADMIN' && (
                                    <Link
                                        to="/admin/dashboard"
                                        className={`flex flex-col items-center gap-1 px-4 py-2 rounded-lg hover:bg-white/10 transition-all duration-200 ${location.pathname === '/admin/dashboard' ? 'border-b-2 border-brand-primary' : ''
                                            }`}
                                    >
                                        <LayoutDashboard size={20} className="text-brand-primary" />
                                        <span className="text-xs text-brand-primary">لوحة التحكم</span>
                                    </Link>
                                )}
                                ...
                                <Link
                                    to="/cart"
                                    className={`flex flex-col items-center gap-1 px-4 py-2 rounded-lg hover:bg-white/10 transition-all duration-200 relative ${location.pathname === '/cart' ? 'border-b-2 border-brand-primary' : ''
                                        }`}
                                >
                                    ...
                                    <span className="text-xs text-white">طلباتي</span>
                                </Link>
                            </div>
                        )}
                        ...
                        {isReviewer ? (
                            ...
                            <Link
                                ...
                                className={`flex flex-col items-center gap-1 px-4 py-2 rounded-lg hover:bg-white/10 transition-all duration-200 ${location.pathname === '/profile' || location.pathname === '/login' ? 'border-b-2 border-brand-primary' : ''
                                    }`}
                            >
                                ...
                                <span className="text-xs text-white">
                                    {user ? user.firstName || 'حسابي' : 'تسجيل'}
                                </span>
                            </Link>
                        )}
                ...
                {isMobileMenuOpen && (
                    <div className="md:hidden bg-[#2c3e50] border-t border-gray-700/50 absolute w-full top-full right-0 shadow-2xl p-4 z-50 text-right animate-in fade-in slide-in-from-top-2">
                        <div className="flex flex-col gap-2 items-center w-full">
                            ...
                            <Link to="/" onClick={() => setIsMobileMenuOpen(false)} className="w-full flex items-center justify-center gap-3 text-white hover:text-[#C17554] hover:bg-white/5 font-bold px-4 py-3 rounded-lg transition-all block">
                                <Home size={20} />
                                <span>الرئيسية</span>
                            </Link>
                            <Link to="/marketplace" onClick={() => setIsMobileMenuOpen(false)} className="w-full flex items-center justify-center gap-3 text-white hover:text-[#C17554] hover:bg-white/5 font-bold px-4 py-3 rounded-lg transition-all block">
                                <BookOpen size={20} />
                                <span>الكتب</span>
                            </Link>
                            <button onClick={() => { scrollToSection('about-us'); setIsMobileMenuOpen(false); }} className="w-full flex items-center justify-center gap-3 text-white hover:text-[#C17554] hover:bg-white/5 font-bold px-4 py-3 rounded-lg transition-all">
                                <Info size={20} />
                                <span>من نحن</span>
                            </button>
                            ...
                            <Link to="/sell" onClick={handleSellClick} className="w-full flex items-center justify-center gap-3 bg-[#C17554] hover:bg-[#a95234] text-white font-bold px-4 py-3 rounded-lg transition-all shadow-md block">
                                <Plus size={20} />
                                <span>عرض كتابك</span>
                            </Link>
                            <Link to="/cart" onClick={() => setIsMobileMenuOpen(false)} className="w-full flex items-center justify-center gap-3 text-white hover:text-[#C17554] hover:bg-white/5 font-bold px-4 py-3 rounded-lg transition-all block">
                                <div className="relative">
